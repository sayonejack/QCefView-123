                                                        ...
                                                        int priority /* = 0*/)
{
    ...
    auto lower_url = url;
    std::transform(lower_url.begin(), lower_url.end(), lower_url.begin(), [](unsigned char c) { return std::tolower(c); });
...
bool
CefViewBrowserClient::TriggerEvent(CefRefPtr<CefBrowser> browser, const int64_t frame_id, const CefRefPtr<CefProcessMessage> msg)
{
    ...
    if (browser) {
        ...
        if (MAIN_FRAME == frame_id) {
            ...
            std::vector<CefString> frameIdentifiers;
            browser->GetFrameIdentifiers(frameIdentifiers);
            for (int i = 0; i < frameIdentifiers.size(); i++) {
                if (i == frame_id) {
                    frameIds.push_back(frameIdentifiers[i]);
                    break;
                }
            }
        }
