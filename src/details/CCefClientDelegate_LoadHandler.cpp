...
void
CCefClientDelegate::loadingStateChanged(CefRefPtr<CefBrowser>& browser, bool isLoading, bool canGoBack, bool canGoForward)
{
...
CCefClientDelegate::loadStart(CefRefPtr<CefBrowser>& browser, CefRefPtr<CefFrame>& frame, int transitionType)
{
    ...
    emit pCefViewPrivate_->q_ptr->loadStart(browser->GetIdentifier(),
...
CCefClientDelegate::loadEnd(CefRefPtr<CefBrowser>& browser, CefRefPtr<CefFrame>& frame, int httpStatusCode)
{
    ...
    emit pCefViewPrivate_->q_ptr->loadEnd(browser->GetIdentifier(),
#if CEF_VERSION_MAJOR > 119
                                          QString::fromStdString(frame->GetIdentifier()),
#else
                                          frame->GetIdentifier(),
