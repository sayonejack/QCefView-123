...
int QCefViewPrivate::currentCursorPosition = 0;
...
QCefViewPrivate::QCefViewPrivate(QCefContextPrivate* ctx, QCefView* view, const QString& url, const QCefSetting* setting)
  : q_ptr(view)
...
bool
QCefViewPrivate::onBeforeNewBrowserCreate(
...
bool
QCefViewPrivate::onBeforeNewPopupCreate(
#if CEF_VERSION_MAJOR > 119
  std::string frameId,
#else
  qint64 frameId,
...
#if CEF_VERSION_MAJOR > 119
                          QString::fromStdString(frame->GetIdentifier()),
#else
                          frame->GetIdentifier(),
#endif
                          frame->IsMain(),
...
QCefViewPrivate::onRunCefContextMenu(QPoint pos, CefRefPtr<CefRunContextMenuCallback> callback)
{
    ...
    QContextMenuEvent* e = new QContextMenuEvent(QContextMenuEvent::Other, pos, globalPos, QGuiApplication::keyboardModifiers());
#endif
    qApp->sendEvent(q, e);
}
...
QCefViewPrivate::onViewInputMethodEvent(QInputMethodEvent* event)
{
    if (isOSRModeEnabled_) {
        ...
        CefRange range;
        // range.Set(0, static_cast<decltype(CefRange::to)>(composedText.length()));
        currentCursorPosition += composedText.length();
        range.Set(static_cast<decltype(CefRange::to)>(currentCursorPosition),
                  static_cast<decltype(CefRange::to)>(currentCursorPosition));
        ...
        if (!composedText.isEmpty()) {
            pCefBrowser_->GetHost()->ImeCommitText(composedText.toStdString(), range, currentCursorPosition);
            ...
            pCefBrowser_->GetHost()->ImeSetComposition(composingText.toStdString(), { underline }, range, selectionRange);
        } else {
...
QCefViewPrivate::onViewWheelEvent(QWheelEvent* event)
{
    if (isOSRModeEnabled_) {
        ...
        pCefBrowser_->GetHost()->SendMouseWheelEvent(e, m & Qt::ShiftModifier ? d.x() : 0, m & Qt::ShiftModifier ? d.y() : d.y());
    }
}
...
bool
QCefViewPrivate::executeJavascript(
#if CEF_VERSION_MAJOR > 119
  std::string frameId,
#else
  int64_t frameId,
...
#if CEF_VERSION_MAJOR > 119
        auto frame = frameId.empty() ? pCefBrowser_->GetMainFrame() : pCefBrowser_->GetFrameByIdentifier(frameId);
#else
...
bool
QCefViewPrivate::executeJavascriptWithResult(
#if CEF_VERSION_MAJOR > 119
  std::string frameId,
#else
  int64_t frameId,
